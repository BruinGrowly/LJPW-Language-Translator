...
# ============================================================================
# Phase 7: Neural Generation
# ============================================================================

class LJPWNeuralGenerator:
    """
    Neural Generator for LJPW -> Text translation.
    
    This model "decodes" pure semantic coordinates into fluent English text.
    It uses Fibonacci layers to expand the compressed 4D meaning into a rich
    latent state, which then conditions a sequence decoder.
    
    Architecture:
    1. Input: 4D LJPW Vector [L, J, P, W]
    2. Semantic Expansion (Fibonacci Stack):
       - Layer 1: 4 -> 89 (F11) [Relu/Swish]
       - Layer 2: 89 -> 233 (F13) [Relu/Swish/Tanh]
    3. Decoder (LSTM/GRU):
       - Initial State: Conditioned on expanded semantic state
       - Input: Previous word embedding
    4. Output: Softmax over vocabulary
    
    Why this matters:
    It allows the system to "speak" from meaning, rather than just
    manipulating text strings. It grounds language in the LJPW reality.
    """
    
    def __init__(self, 
                 vocab_size: int,
                 embedding_dim: int = 256,
                 hidden_dim: int = 233,  # F13
                 expansion_layers: List[int] = [11, 13],  # F11(89) -> F13(233)
                 dropout: float = 0.2):
        
        self.vocab_size = vocab_size
        self.hidden_dim = hidden_dim
        
        # 1. Semantic Expansion (LJPW -> Latent)
        # Using Fibonacci layers for natural expansion
        self.expansion = []
        input_dim = 4
        for fib_idx in expansion_layers:
            layer = FibonacciLayer(input_dim, fib_idx, activation='swish')
            self.expansion.append(layer)
            input_dim = layer.size
            
        self.latent_dim = input_dim
        
        # 2. Decoder Components (Mock-up for pure Python implementation)
        # In a real framework like PyTorch/TF, these would be nn.Module
        # Here we define the structure for the "Natural" implementation
        
        # Embedding: Word -> Vector
        self.embedding_weights = np.random.randn(vocab_size, embedding_dim) * 0.01
        
        # Recurrent Cell (Simplified LSTM logic for structure)
        # W_f, W_i, W_c, W_o
        self.lstm_weights = np.random.randn(embedding_dim + hidden_dim, 4 * hidden_dim) * 0.01
        self.lstm_bias = np.zeros(4 * hidden_dim)
        
        # Output Projection: Hidden -> Vocab
        self.output_weights = np.random.randn(hidden_dim, vocab_size) * 0.01
        self.output_bias = np.zeros(vocab_size)
        
        print(f"Initialized LJPWNeuralGenerator (Vocab: {vocab_size}, Latent: {self.latent_dim})")

    def forward_step(self, 
                    x_t: np.ndarray, 
                    h_t: np.ndarray, 
                    c_t: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """
        Single step of LSTM decoder.
        x_t: Input embedding
        ...
        gates = concat @ self.lstm_weights + self.lstm_bias
        
        # Split gates
        chunk_size = self.hidden_dim
        f_gate = self._sigmoid(gates[:, :chunk_size])
        i_gate = self._sigmoid(gates[:, chunk_size:2*chunk_size])
        c_cand = np.tanh(gates[:, 2*chunk_size:3*chunk_size])
        o_gate = self._sigmoid(gates[:, 3*chunk_size:])
        
        # Update cell state
        c_next = f_gate * c_t + i_gate * c_cand
    ...
    def decode(self, 
               ljpw_vector: np.ndarray, 
               max_length: int = 20,
               start_token: int = 1) -> List[int]:
        """
        Generate sequence from LJPW vector.
        """
        # 1. Expand Semantic Vector
        z = ljpw_vector
        for layer in self.expansion:
            z = layer.forward(z, training=False)
        
        # Initialize Decoder State (Conditioning)
        # In a full model, we might have a bridge layer z -> (h0, c0)
        # For now, simplistic mapping if dimensions match, else projection
        h_t = np.zeros((1, self.hidden_dim))
        c_t = np.zeros((1, self.hidden_dim))
        
        # Project z to initialize h_t if needed (simplified)
        ...
        for _ in range(max_length):
            # Embed current token
            # x_t = self.embedding_weights[current_token] # simplified
            x_t = np.expand_dims(self.embedding_weights[current_token], 0)
            
            # RNN Step
            h_t, c_t, output = self.forward_step(x_t, h_t, c_t)
            
            # Project to Vocab
            logits = h_t @ self.output_weights + self.output_bias
            probs = self._softmax(logits)
            
            # Greedy decode (take max)
            next_token = np.argmax(probs)
            generated_tokens.append(int(next_token))
            current_token = next_token
            
            if next_token == 0: # End token assumption
                break
...
# ============================================================================
# Phase 7.1: Multi-Scale Neural Generation (Fractal Profiling)
# ============================================================================

class LJPWMultiScaleGenerator:
    """
    Multi-Scale Neural Generator for LJPW -> Text translation.
    
    Implements "Fractal Profiling" from LJPW Codex (Section 8):
    - Entity Level: Verse meaning (4D)
    - Cluster Level: Chapter context (4D)
    - System Level: Narrative flow (4D)
    
    Total Input: 12D semantic context
    
    Architecture:
    1. Input: 12D Context Vector [Verse + Chapter + Narrative]
    2. Semantic Expansion (Fibonacci Stack):
       - Layer 1: 12 -> 144 (F12) [Swish]
       - Layer 2: 144 -> 377 (F14) [Swish]
    3. Decoder (LSTM):
       - Initial State: Conditioned on 377D latent state
    4. Output: Softmax over vocabulary
    
    Why this matters:
    The 12D input mitigates the "Information Bottleneck" by providing
    multi-scale context, allowing the model to generate historically
    accurate and contextually coherent text.
    """
    ...
    def __init__(self, 
                 ...
                 expansion_layers: List[int] = [12, 14],  # F12(144) -> F14(377)
                 dropout: float = 0.2):
        
        self.vocab_size = vocab_size
        self.hidden_dim = hidden_dim
        
        # 1. Semantic Expansion (12D Context -> Latent)
        self.expansion = []
        input_dim = 12  # 3 x 4D vectors
        for fib_idx in expansion_layers:
            layer = FibonacciLayer(input_dim, fib_idx, activation='swish')
            self.expansion.append(layer)
            input_dim = layer.size
        ...
        # 3. Bridge layer to project latent to LSTM initial state
        self.bridge_weights = np.random.randn(self.latent_dim, hidden_dim) * 0.01
        self.bridge_bias = np.zeros(hidden_dim)
        
        print(f"Initialized LJPWMultiScaleGenerator (Vocab: {vocab_size}, Latent: {self.latent_dim})")
    ...
    def decode(self, 
               verse_vector: np.ndarray,
               chapter_context: np.ndarray,
               narrative_flow: np.ndarray,
               max_length: int = 20,
               start_token: int = 1) -> List[int]:
        """
        Generate sequence from multi-scale LJPW context.
        
        Args:
            verse_vector: 4D LJPW vector for the specific verse
            chapter_context: 4D LJPW vector for the chapter theme
            narrative_flow: 4D LJPW vector from the previous verse
        """
        # 1. Concatenate multi-scale context
        context = np.concatenate([verse_vector, chapter_context, narrative_flow], axis=1)
        
        # 2. Expand through Fibonacci layers
        z = context
        for layer in self.expansion:
            z = layer.forward(z, training=False)
        
        # 3. Initialize Decoder State via Bridge
        h_t = np.tanh(z @ self.bridge_weights + self.bridge_bias)
        c_t = np.zeros((1, self.hidden_dim))

"""
Test Simple Translator with Action Concepts
"""
...
def load_concepts():
    """Load concepts from actions library."""
    with open("experiments/semantic_space_6674_ACTIONS.json", 'r', encoding='utf-8') as f:
        data = json.load(f)
...
def detect_field_position(text):
    """Simple pattern detection for testing actions."""
    ...
    if 'build' in text_lower:
        L=0.7; J=0.9; P=0.8; W=0.8
    elif 'destroy' in text_lower:
        L=0.1; J=0.1; P=0.9; W=0.2
    elif 'run' in text_lower:
        P=0.8; L=0.5; J=0.5; W=0.4
    elif 'pray' in text_lower:
        L=0.9; P=0.3; J=0.7; W=0.9
    elif 'think' in text_lower:
        W=0.8; L=0.5; J=0.6; P=0.4
    elif 'fight' in text_lower:
        L=0.1; J=0.2; P=0.9; W=0.3
...
def main():
    print("="*70)
    print("TESTING ACTION CONCEPTS")
    print("="*70)
    
    concepts = load_concepts()
    print(f"\nLoaded {len(concepts)} concepts (Spiritual + Emotional + Relations + Actions)")
